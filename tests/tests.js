exports.defineAutoTests = function() {
  ...
  describe('DBMeter (window.DBMeter)', function () {
    it('should exist', function() {
      expect(window.AudioBuffer).toBeDefined();
    });

    it('should contain a start function', function () {
      expect(typeof window.AudioBuffer.start).toBeDefined();
      expect(typeof window.AudioBuffer.start === 'function').toBe(true);
    });

    it('should contain a stop function', function () {
      expect(typeof window.AudioBuffer.stop).toBeDefined();
      expect(typeof window.AudioBuffer.stop === 'function').toBe(true);
    });

    it('should contain a isListening function', function () {
      expect(typeof window.AudioBuffer.isListening).toBeDefined();
      expect(typeof window.AudioBuffer.isListening === 'function').toBe(true);
    });

    it('should contain a delete function', function () {
      expect(typeof window.AudioBuffer.delete).toBeDefined();
      expect(typeof window.AudioBuffer.delete === 'function').toBe(true);
    });
  });

  describe('start method', function () {

    it('should start the AudioBuffer', function (done) {
      var started = false;

      window.AudioBuffer.start(function() {
        if(!started) {
          started = true;
          window.AudioBuffer.isListening(function(result) {
    ...
    it('should return dB if we start the AudioBuffer', function (done) {
      var started = false;

      window.AudioBuffer.delete(function() {
        window.AudioBuffer.start(function(dB) {
          if(!started) {
            started = true;
  ...
  describe('stop method', function () {
    ...
    it('should stop the DBMeter', function (done) {
      ...
      window.AudioBuffer.delete(function() {
        window.AudioBuffer.start(function() {
          if(!started) {
            started = true;
            window.AudioBuffer.stop(function() {
              window.AudioBuffer.isListening(function(result) {
    ...
    describe('error callback', function () {
      ...
      it('should be called if we stop the DBMeter while it is not started, the expected error should have the code 1', function (done) {
        window.AudioBuffer.delete(function() {
          window.AudioBuffer.stop(fail.bind(null, done), function(e) {
  ...
  describe('delete method', function () {
    ...
    describe('error callback', function () {
      it('should be called if we delete the DBMeter while it is not initialized, the expected error should have the code 0', function (done) {
        window.AudioBuffer.delete(fail.bind(null, done), function(e) {
    ...
    it('should return true if the DBMeter is listening', function (done) {
      ...
      window.AudioBuffer.start(function() {
        if(!started) {
          ...
          window.AudioBuffer.stop();
          window.AudioBuffer.delete();
        }
      }, fail.bind(null, done));
    });
  });
